...
class CommanderGUI:
    ...
    def handle_location_request(self, conn, tank_id):
        ...
        try:
            ...
            while True:
                encrypted_payload = conn.recv(1024).decode()
                if not encrypted_payload:
                    break
                
                # Decrypt and process location
                location = self.decrypt_location(encrypted_payload, tank_id)
                if location:
                    self.update_tank_location(tank_id, location)
        
        except Exception as e:
            self.log(f"Location handling error for Tank {tank_id}: {e}", level="ERROR")
    ...
    def decrypt_location(self, encrypted_payload, tank_id):
        ...
        try:
            ...
            key_aes, key_des, key_tdes, private_key_rsa, public_key_rsa, private_key_ecc, public_key_ecc = get_keys_by_index(index)
            ...
            if not is_valid:
                self.log(f"Invalid signature from Tank {tank_id}", level="WARNING")
                return None
                
            return decrypted_location
            
        except Exception as e:
            self.log(f"Decryption error for Tank {tank_id}: {e}", level="ERROR")
            return None
